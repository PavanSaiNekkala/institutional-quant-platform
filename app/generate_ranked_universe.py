...
universe = pd.read_excel("updated_stocks.xlsx")
...
for idx, symbol in enumerate(symbols):
    try:
        ...
        data = yf.download(symbol, period="3mo", progress=False, auto_adjust=True, threads=True)
...
ranking_df["Institutional Score"] = pd.to_numeric(
    ranking_df["Institutional Score"], errors="coerce"
)

ranking_df = ranking_df.dropna(subset=["Institutional Score"])
...
ranking_df.to_excel("ranked_universe.xlsx", index=False)
...
print(f"TOTAL RANKED STOCKS: {len(ranking_df)}")
