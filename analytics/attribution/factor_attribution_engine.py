...
for df in [portfolio, factor_df, expected_df]:
...
df["MOMENTUM_ATTR"] = df[weight_col] * df["FACTOR_MOMENTUM"]

df["SHARPE_ATTR"] = df[weight_col] * df["FACTOR_SHARPE"]

df["ALPHA_ATTR"] = df[weight_col] * df["FACTOR_ALPHA"]

df["RS_ATTR"] = df[weight_col] * df["FACTOR_RS"]

df["SECTOR_ATTR"] = df[weight_col] * df["FACTOR_SECTOR"]

df["ENTRY_ATTR"] = df[weight_col] * df["FACTOR_ENTRY"]

df["LIQUIDITY_ATTR"] = df[weight_col] * df["FACTOR_LIQUIDITY"]

df["EXPECTED_RETURN_ATTR"] = df[weight_col] * df["EXPECTED_RETURN_30D"]
...
summary["Contribution"] = summary["Contribution"].round(4)

summary = summary.sort_values(by="Contribution", ascending=False).reset_index(drop=True)

# =========================================================
# SAVE SUMMARY
# =========================================================

summary.to_csv(OUTPUT_ATTRIBUTION, index=False)
...
stock_output.to_csv(OUTPUT_STOCK_ATTRIBUTION, index=False)
...
print(OUTPUT_ATTRIBUTION)

print(OUTPUT_STOCK_ATTRIBUTION)
