...
required_cols = ["Symbol", "Sector", "Momentum", "Sharpe", "ALPHA_SCORE"]
...
if ENTRY_FILE.exists():
    ...
    entry_df["Symbol"] = (
        entry_df["Symbol"].astype(str).str.upper().str.replace(".NS", "", regex=False)
    )

    df = df.merge(entry_df[["Symbol", "ENTRY_SCORE"]], on="Symbol", how="left")

    print("\nAFTER MERGE")

    print(df[["Symbol", "ENTRY_SCORE"]].head(20))

    print("\nNon Zero After Merge:", (df["ENTRY_SCORE"].fillna(0) > 0).sum())
...
print("\nNon Zero Before Factor Engine:", (df["ENTRY_SCORE"].fillna(0) > 0).sum())
...
df["FACTOR_MOMENTUM"] = df["Momentum"].rank(pct=True) * 100

# ---------------------------------------------------------
# SHARPE FACTOR
# ---------------------------------------------------------

df["FACTOR_SHARPE"] = df["Sharpe"].rank(pct=True) * 100

# ---------------------------------------------------------
# ALPHA FACTOR
# ---------------------------------------------------------

df["FACTOR_ALPHA"] = df["ALPHA_SCORE"].rank(pct=True) * 100
...
df["FACTOR_RS"] = df["RS_COMPOSITE"].rank(pct=True) * 100
...
if "VOL_ADJ_RS" in df.columns:
    df["FACTOR_LOW_VOL"] = (1 / (df["VOL_ADJ_RS"] + 1e-6)).rank(pct=True) * 100
...
if "SECTOR_SCORE" in df.columns:
    df["FACTOR_SECTOR"] = df["SECTOR_SCORE"].rank(pct=True) * 100
...
df = df.sort_values(by="MULTI_FACTOR_SCORE", ascending=False)

df["FACTOR_RANK"] = range(1, len(df) + 1)
...
print(df[["FACTOR_RANK", "Symbol", "Sector", "MULTI_FACTOR_SCORE"]].head(20))
