...
required_cols = ["Symbol", "FINAL_WEIGHT"]
...
var95 = np.percentile(portfolio_returns, 5) * 100

cvar95 = portfolio_returns[portfolio_returns <= np.percentile(portfolio_returns, 5)].mean() * 100
...
largest_position = df["FINAL_WEIGHT"].max()

top5 = df["FINAL_WEIGHT"].nlargest(5).sum()

top10 = df["FINAL_WEIGHT"].nlargest(10).sum()
...
if largest_position > MAX_POSITION:
    alerts.append("POSITION_TOO_LARGE")

if top5 > MAX_TOP5:
    alerts.append("TOP5_CONCENTRATION")
...
health -= max(0, annual_vol - 20)

health -= abs(min(0, max_dd)) * 0.5

health -= max(0, (largest_position * 100) - 10)
...
pd.DataFrame({"ALERT": alerts}).to_csv(OUTPUT_ALERTS, index=False)
