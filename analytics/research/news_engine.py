...
updated_stocks_FILE = "data/raw/updated_stocks.xlsx"

OUTPUT_FILE = "data/news_rankings.csv"

CACHE_FILE = ROOT / "data" / "news_cache.parquet"

HISTORY_FILE = ROOT / "data" / "news_history.parquet"
...
valid_df = pd.read_excel(updated_stocks_FILE)

stocks = valid_df["Symbol"].dropna().astype(str).unique().tolist()
...
df["NEWS_ALPHA"] = df["NEWS_SCORE"] * 8 + df["NEWS_COUNT"] * 2

scaler = MinMaxScaler()

df["NEWS_ALPHA"] = (scaler.fit_transform(df[["NEWS_ALPHA"]]) * 100).round(2)
