...
symbols = (
    rank_df["Symbol"].astype(str).str.replace(".NS", "", regex=False).str.strip().unique().tolist()
)

symbols = [s for s in symbols if s not in ["", "nan", "None"]]
...
prices = yf.download(
    tickers=tickers, period=LOOKBACK_PERIOD, auto_adjust=True, progress=False, threads=True
)

print(f"\nDownloaded Columns: {len(prices.columns)}")
...
if isinstance(prices.columns, pd.MultiIndex):
    if "Close" in prices.columns.levels[0]:
        ...
        raise Exception("Unable to locate Close prices.")
...
if BENCHMARK not in prices.columns:
    ...
    benchmark_df = yf.download(BENCHMARK, period=LOOKBACK_PERIOD, auto_adjust=True, progress=False)
    ...
    if benchmark_df.empty:
        ...
        pd.DataFrame().to_csv(EQUITY_CURVE_FILE, index=False)
...
for start in range(TRAIN_WINDOW, len(returns) - TEST_WINDOW, TEST_WINDOW):
    ...
    for col in train_data.columns:
        ...
        rs_score = cumulative_return / volatility
    ...
    top_stocks = [x[0] for x in ranked[:TOP_N]]
    ...
    for stock in top_stocks:
        ...
        stock_returns = test_data[stock].replace([np.inf, -np.inf], np.nan).fillna(0)
    ...
    cumulative_test_return = (1 + portfolio_returns).prod() - 1

    if not np.isfinite(cumulative_test_return):
        continue

    capital *= 1 + cumulative_test_return
...
equity_curve = pd.DataFrame({"Date": dates, "Portfolio_Value": portfolio_values})

# =========================================================
# VALIDATION
# =========================================================
updated_stocks = [c for c in prices.columns if c != BENCHMARK]
...
years = max((len(equity_curve) * TEST_WINDOW / TRADING_DAYS), 0.1)

cagr = (1 + total_return) ** (1 / years) - 1

volatility = equity_curve["Returns"].std() * np.sqrt(TRADING_DAYS)
...
drawdown = equity_curve["Portfolio_Value"] / rolling_max - 1

max_drawdown = drawdown.min()

win_rate = (equity_curve["Returns"] > 0).mean() * 100
...
benchmark_total_return = benchmark_curve.iloc[-1] - 1
