...
if "FINAL_WEIGHT" not in portfolio.columns:
    raise ValueError("FINAL_WEIGHT column missing.")
...
weights = portfolio["FINAL_WEIGHT"].values
...
prices = yf.download(tickers=symbols, period=f"{LOOKBACK_YEARS}y", auto_adjust=True, progress=False)
...
benchmark = yf.download(BENCHMARK, period=f"{LOOKBACK_YEARS}y", auto_adjust=True, progress=False)
