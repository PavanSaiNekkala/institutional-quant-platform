...
universe = universe.sort_values("MULTI_FACTOR_SCORE", ascending=False)

print(f"\n📊 Total Universe Stocks: {len(universe)}")
...
if "YF_SYMBOL" in universe.columns:
    symbols = universe["YF_SYMBOL"].dropna().astype(str).unique().tolist()

elif "Symbol" in universe.columns:
    symbols = (
        (
            universe["Symbol"].dropna().astype(str).str.upper().str.replace(".NS", "", regex=False)
            + ".NS"
        )
        .unique()
        .tolist()
...
close_df = close_df.loc[:, valid_history]

print(f"\nStocks with 200d history: {close_df.shape[1]}")
...
if len(close_df) < 200:
    raise ValueError("Insufficient history for breadth calculations.")

previous_close = close_df.iloc[-2]

advancers = (latest_close > previous_close).sum()

decliners = (latest_close < previous_close).sum()

ad_ratio = (advancers / max(advancers + decliners, 1)) * 100
...
lookback_days = min(252, len(close_df))

high_52w = close_df.rolling(lookback_days).max()

valid_highs = latest_close.notna() & high_52w.iloc[-1].notna()

pct_new_highs = (
    (latest_close[valid_highs] >= high_52w.iloc[-1][valid_highs]).sum() / max(valid_highs.sum(), 1)
) * 100
...
valid_50 = latest_close.notna() & latest_ma50.notna()

valid_200 = latest_close.notna() & latest_ma200.notna()

pct_above_50 = (
    (latest_close[valid_50] > latest_ma50[valid_50]).sum() / max(valid_50.sum(), 1)
) * 100

pct_above_200 = (
    (latest_close[valid_200] > latest_ma200[valid_200]).sum() / max(valid_200.sum(), 1)
) * 100
...
breadth_score = pct_above_200 * 0.40 + pct_above_50 * 0.30 + ad_score * 0.20 + pct_new_highs * 0.10

breadth_score = round(min(100, max(0, breadth_score)), 2)
...
output = pd.DataFrame(
    {
        ...
        "ADVANCE_PERCENTAGE": [round(ad_ratio, 2)],
        "PCT_NEW_52W_HIGHS": [round(pct_new_highs, 2)],
        "BREADTH_SCORE": [breadth_score],
        "BREADTH_REGIME": [breadth_regime],
        "STOCKS_ANALYSED": [int(valid_200.sum())],
    }
)
...
print(f"Above 200DMA: {pct_above_200:.2f}%")

print(f"Advance %      : {ad_ratio:.2f}")

print(f"52W Highs %   : {pct_new_highs:.2f}%")

print(f"Stocks Used   : {int(valid_200.sum())}")

print(f"Breadth Score: {breadth_score}")

print(f"Breadth Regime: {breadth_regime}")
