...
def load_symbols():
    ...
    try:
        ...
        symbols = df[stock_col].dropna().astype(str).str.upper().str.strip().unique().tolist()
...
def prefilter_symbols(symbols):
    ...
    blacklist = ["ETF", "BEES", "GOLD", "SILVER", "LIQUID", "INDEX", "SETF", "JUNIORBEES"]

    for s in symbols:
        if any(x in s.upper() for x in blacklist):
            continue
