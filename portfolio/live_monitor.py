...
def fetch_returns(symbol, period="3mo"):
...
def live_portfolio_report(symbols, weights):

    returns_data = {}

    for symbol in symbols:
        returns_data[symbol] = fetch_returns(symbol)

    returns_df = pd.DataFrame(returns_data).dropna()

    port_vol = portfolio_volatility(returns_df, weights)

    port_return = portfolio_return(returns_df, weights)

    sharpe = port_return / port_vol

    report = {
        "Portfolio Return": round(port_return, 4),
        "Portfolio Volatility": round(port_vol, 4),
