...
class PNLEngine:
    def __init__(self, trades):
        ...
        if not self.trades.empty:
            self.trades.columns = [str(col).strip().lower() for col in self.trades.columns]
    ...
    def position_analysis(self):
        ...
        for symbol in symbols:
            df = self.trades[self.trades["symbol"] == symbol]
            ...
            realized_pnl = sell_value - (avg_buy_price * sell_qty)
            ...
            if current_price is not None and net_qty > 0:
                market_value = current_price * net_qty

                unrealized_pnl = market_value - (avg_buy_price * net_qty)

            results.append(
                {
                    "Symbol": symbol,
                    "Net Quantity": round(net_qty, 2),
    ...
    def total_pnl(self):
        ...
        realized = df["Realized PnL"].sum()

        unrealized = df["Unrealized PnL"].sum()

        total = realized + unrealized

        return {
            "Total Realized PnL": round(realized, 2),
            "Total Unrealized PnL": round(unrealized, 2),
            "Total Portfolio PnL": round(total, 2),
        }
