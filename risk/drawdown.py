...
def rolling_max_drawdown(equity_curve, window=252):
    ...
    for i in range(window, len(equity_curve)):
        subset = equity_curve.iloc[i - window : i]

        dd = maximum_drawdown(subset)

        rolling_dd.append(dd)

    return pd.Series(rolling_dd, index=equity_curve.index[window:])
...
def underwater_curve(equity_curve):
    ...
    underwater = (equity_curve - rolling_max) / rolling_max
