...
def train_regime_ai(prices):

    df = build_features(prices)

    regime = detect_regime(df["returns"])

    X = df[["volatility", "momentum", "moving_avg"]]

    y = df["target"]

    model = regime_model(regime)

    model.fit(X, y)

    prediction = model.predict(X.tail(1))[0]

    return {"Regime": regime, "Prediction": prediction, "Model": model.__class__.__name__}
