...
def ai_forecast(symbol, period="6mo"):
    ...
    try:
        ...
        forecast_return = returns.tail(10).mean()

        volatility = returns.std() * np.sqrt(252)

        forecast_price = close.iloc[-1] * (1 + forecast_return)
        ...
        return {
            ...
            "Forecast Return": round(forecast_return, 4),
...
class DistributedForecasting:
    ...
    def run_forecasts(self, symbols, workers=8):

        self.queue.start_workers(num_workers=workers)

        for symbol in symbols:
            self.queue.add_task(ai_forecast, symbol)
