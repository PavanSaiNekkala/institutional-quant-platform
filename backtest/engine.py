...
def build_features(df):
    ...
    df["distance_from_high"] = distance_from_high(df["close"])
    ...
    df["trend_quality"] = trend_quality(df)

    # =====================================================
    # VOLATILITY CONTRACTION
    # =====================================================

    df["volatility_contraction"] = volatility_contraction(df)
...
def run_backtest(stock_data):
    ...
    top_stocks = scores_df.head(10)["symbol"].tolist()
...
def simulate_portfolio(stock_data, top_stocks, initial_capital=100000):
    ...
    benchmark = yf.download("^NSEI", period="2y", progress=False)
    ...
    weights = np.repeat(1 / len(valid_symbols), len(valid_symbols))
    ...
    benchmark_cumulative = (benchmark_equity.iloc[-1] / initial_capital) - 1

    benchmark_annualized = ((1 + benchmark_cumulative) ** (252 / len(benchmark_returns))) - 1
    ...
    sortino_ratio = annualized_return / (downside_std + 1e-9)
    ...
    risk_adjusted_return = cumulative_return / (annualized_volatility + 1e-9)
    ...
    results = {
        ...
        "Final Portfolio Value": equity_curve.iloc[-1],
        ...
        "Benchmark Return": benchmark_annualized,
        "Alpha": alpha,
        "Maximum Drawdown": max_drawdown,
    }

    return (portfolio_returns, equity_curve, benchmark_equity, results)
